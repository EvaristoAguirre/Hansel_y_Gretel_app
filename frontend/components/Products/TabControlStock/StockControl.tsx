...
const StockControl = () => {
  ...
  const [simpleAndComposedProducts, setSimpleAndComposedProducts] = useState<
    ProductCreated[]
  >([]);
  const [simpleProducts, setSimpleProducts] = useState<ProductCreated[]>([]);
  ...
  const filterByComposition = (products: ProductCreated[]) => {
    // Filtrar productos que sean de tipo "PRODUCT"
    const filteredProducts = products.filter(
      (product) => product.type === TypeProduct.PRODUCT
    );
  
    // Filtrar productos que NO tengan ingredientes
    const simpleProducts = filteredProducts.filter(
      (product) => !product.productIngredients || product.productIngredients.length === 0
    );
  
    setSimpleAndComposedProducts(filteredProducts);
    setSimpleProducts(simpleProducts);
  };

  const formattedProducts = simpleProducts.map((product) => ({
    id: product.id,
    name: product.name,
  ...
  const filterByStock = (items: SelectedItem[]) => {
    return items.filter((item: SelectedItem) => {
      ...
      if (selectedStockFilter === "medium")
        return stockQuantity > minStock / 2 && stockQuantity <= minStock;
  ...
  useEffect(() => {
    ...
    const hasNullStockIngredients = filteredIngredients.some(
      (ingredient) =>
  ...
  return (
    <Box width="100%" sx={{ p: 2, minHeight: "100vh" }}>
      <FilterStock
        currentFilter={selectedStockFilter}
        onFilterChange={setSelectedStockFilter}
      />
      {noStock && (
        <Typography variant="h6" sx={{ mt: 2 }} color="error">
          ❗️Hay productos o ingredientes sin stock asignado. Seleccione la fila
          correspondiente en la tabla para agregar stock.
        </Typography>
      )}
      {/* DataGrids */}
      <Box display="flex" gap={2} sx={{ mt: 3 }}>
        ...
        <Box flex={1}>
          ...
          <DataGrid
            ...
            onRowClick={(params) =>
              handleEditProduct(params.row, StockModalType.PRODUCT)
            }
          />
        </Box>
