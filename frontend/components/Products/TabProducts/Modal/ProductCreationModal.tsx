...
const ProductCreationModal: React.FC<ProductCreationModalProps> = ({
  ...
  const [tabValue, setTabValue] = useState<TabProductKey>(
  ...
  const isToppingsSectionValid = (): boolean => {
    ...
    if (
      !Array.isArray(form.availableToppingGroups) ||
      form.availableToppingGroups.length === 0
    ) {
      return false;
    }
  ...
  const handleSaveProduct = async () => {
    if (tabValue === TabProductKey.COMBO && onSaveCombo) {
      await onSaveCombo();
    } else {
      onSave();
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <Tab
            label="Promo + Slot"
            value={TabProductKey.COMBO}
            disabled={onDisableTabs(TabProductKey.COMBO)}
          />
        </Tabs>
        ...
        {tabValue === TabProductKey.PRODUCT_WITH_INGREDIENT && (
          <IngredientDialog
            onSave={handleSaveIngredients}
            form={form}
            units={units}
            handleSetDisableTabs={handleSetDisableTabs}
          />
        )}
        {tabValue === TabProductKey.PROMO && (
          <InputsPromo
            onSave={handleProductsPromo}
            form={form}
            handleSetDisableTabs={handleSetDisableTabs}
          />
        )}

        {tabValue === TabProductKey.COMBO && (
          <InputsPromoWithSlots
            onSave={handleSlotsPromo}
            form={form}
            handleSetDisableTabs={handleSetDisableTabs}
          />
        )}
        ...
        {(tabValue === TabProductKey.SIMPLE_PRODUCT ||
          tabValue === TabProductKey.PRODUCT_WITH_INGREDIENT) && (
          <>
            <CheckAllowsToppings
