...
const OrderProvider = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log('🔌 [OrderProvider] WebSocket conectado?', webSocketService.isConnected());
  ...
  const fetchOrderBySelectedTable = useCallback(async () => {
    ...
    console.log('📥 [fetchOrderBySelectedTable] Mesa seleccionada:', selectedTable?.id);
    console.log('📥 [fetchOrderBySelectedTable] Estado de la mesa (selectedTable):', selectedTable?.state);
    console.log('📥 [fetchOrderBySelectedTable] Órdenes de la mesa:', selectedTable?.orders);
    ...
    console.log('📥 [fetchOrderBySelectedTable] Estado de la mesa (actualizado):', actualTableState);
    ...
    if (
      ...
      console.log('📥 [fetchOrderBySelectedTable] Mesa disponible o cerrada, limpiando orden');
      setSelectedOrderByTable(null);
      setConfirmedProducts([]);
      return;
    }
    if (selectedTable) {
      try {
        ...
        if (tableWithOrders?.orders && tableWithOrders.orders.length > 0) {
          ...
          console.log('📥 [fetchOrderBySelectedTable] Orden encontrada en mesa:', orderId);
        } else {
          // Si no hay orders en la mesa, buscar la orden por tableId en el store de órdenes
          const orderInStore = orders.find(o => o.table?.id === selectedTable.id);
          if (orderInStore) {
            orderId = orderInStore.id;
            console.log('📥 [fetchOrderBySelectedTable] Orden encontrada en store:', orderId);
          } else {
            console.log('📥 [fetchOrderBySelectedTable] No se encontró orden. Buscando por tableId en backend...');
            ...
            try {
              ...
              if (ordersResponse.ok) {
                ...
                if (ordersData && ordersData.length > 0) {
                  ...
                  const pendingOrder = ordersData.find((o: IOrderDetails) => o.state === 'pending_payment');
                  orderId = pendingOrder?.id || ordersData[0]?.id;
                  console.log('📥 [fetchOrderBySelectedTable] Orden obtenida desde backend:', orderId);
                }
              }
            } catch (error) {
              console.error('📥 [fetchOrderBySelectedTable] Error al obtener órdenes por tableId:', error);
            }
          }
        }

        if (orderId) {
          console.log('📥 [fetchOrderBySelectedTable] Obteniendo orden:', orderId);
          ...
          if (response.ok) {
            ...
            console.log('📥 [fetchOrderBySelectedTable] Productos en orden:', data.products?.length);
            ...
            if (productsByOrder && productsByOrder.length > 0) {
              console.log('📥 [fetchOrderBySelectedTable] Adaptando productos...');
              ...
              const adaptedProducts: SelectedProductsI[] = productsByOrder.map((product: any) => ({
                productId: product.productId,
                productName: product.productName,
                quantity: product.quantity,
                unitaryPrice: product.unitaryPrice != null ? String(product.unitaryPrice) : null,
              ...
              console.log('📥 [fetchOrderBySelectedTable] Productos adaptados:', adaptedProducts.length);
              handleSetProductsByOrder(adaptedProducts);
            } else {
              console.log('📥 [fetchOrderBySelectedTable] No hay productos en la orden');
              setConfirmedProducts([]);
            }
          } else {
            console.error('📥 [fetchOrderBySelectedTable] Error al obtener la orden:', response.status);
            setSelectedOrderByTable(null);
            setConfirmedProducts([]);
          }
        } else {
          console.log('📥 [fetchOrderBySelectedTable] No se encontró orden para la mesa');
          setSelectedOrderByTable(null);
          setConfirmedProducts([]);
        }
      } catch (error) {
        console.error('📥 [fetchOrderBySelectedTable] Error al obtener el pedido:', error);
        setSelectedOrderByTable(null);
        setConfirmedProducts([]);
      }
    } else {
  ...
  useEffect(() => {
    ...
    const actualTableState = tables.find(t => t.id === selectedTable?.id)?.state || selectedTable?.state;
    if (actualTableState === TableState.AVAILABLE || actualTableState === TableState.CLOSED) {
      console.log('📥 [fetchOrderBySelectedTable] Mesa en estado AVAILABLE/CLOSED, saltando fetch');
      return;
    }
    fetchOrderBySelectedTable();
  ...
  useEffect(() => {
    ...
    console.log('🔔 [orderTicketPrinted] WebSocket conectado?', webSocketService.isConnected());
    ...
    socket.onAny((eventName, ...args) => {
      ...
      if (eventName === 'orderTicketPrinted') {
        console.log('🔍 [WebSocket Debug] ⚡⚡⚡ EVENTO orderTicketPrinted DETECTADO ⚡⚡⚡');
      }
    });

          const handleTicketPrinted = async (data: any) => {
            console.log('🔔 [orderTicketPrinted] ⚡⚡⚡ EVENTO RECIBIDO ⚡⚡⚡', data);
            console.log('🔔 [orderTicketPrinted] Tipo de data:', typeof data);
            console.log('🔔 [orderTicketPrinted] Data completa:', JSON.stringify(data, null, 2));
      ...
      console.log('🔔 [orderTicketPrinted] Evento recibido:', { orderId, orderTableId });
      console.log('🔔 [orderTicketPrinted] Mesa seleccionada:', selectedTable?.id);
      console.log('🔔 [orderTicketPrinted] Orden seleccionada:', selectedOrderByTable?.id);
      console.log('🔔 [orderTicketPrinted] Órdenes en store:', orders.map(o => o.id));
      ...
      console.log('🔔 [orderTicketPrinted] ¿Pertenece a mesa seleccionada?', belongsToSelectedTable);
      ...
      if (belongsToSelectedTable && token) {
        try {
          ...
          if (response.ok) {
            ...
            console.log('🔔 [orderTicketPrinted] Productos en orden:', orderData.products?.length);
            ...
            console.log('🔔 [orderTicketPrinted] Condiciones ANTES de actualizar:', {
              isSelectedOrder,
              isSelectedTable,
              orderBelongsToSelectedTable,
            ...
            if (shouldUpdateSelectedOrder) {
              ...
              console.log('🔔 [orderTicketPrinted] Estado que se establecerá:', newState);
              ...
              console.log('🔔 [orderTicketPrinted] selectedOrderByTable actualizado con estado:', newState);
            } else {
              console.log('🔔 [orderTicketPrinted] NO se actualiza selectedOrderByTable. Razón:', {
                isSelectedOrder,
                isSelectedTable,
                orderBelongsToSelectedTable
              });
            ...
            console.log('🔔 [orderTicketPrinted] Condiciones de actualización de productos:', {
              isSelectedOrder,
              isSelectedTable,
              inOrdersStore: orders.some(o => o.id === orderId),
              orderTableInStore,
            ...
            if (orderData.products && orderData.products.length > 0 && shouldUpdateProducts) {
              console.log('🔔 [orderTicketPrinted] Actualizando confirmedProducts con', orderData.products.length, 'productos');
              // Adaptar ProductLineDto[] a SelectedProductsI[]
              // El backend devuelve ProductLineDto con unitaryPrice: number
              // pero SelectedProductsI espera unitaryPrice?: string | null
              const adaptedProducts: SelectedProductsI[] = orderData.products.map((product: any) => ({
                productId: product.productId,
                productName: product.productName,
                quantity: product.quantity,
                unitaryPrice: product.unitaryPrice != null ? String(product.unitaryPrice) : null,
              ...
              console.log('🔔 [orderTicketPrinted] Productos adaptados:', adaptedProducts.length);
              handleSetProductsByOrder(adaptedProducts);
            } else {
              console.log('🔔 [orderTicketPrinted] No se actualizan productos. Razón:', {
                hasProducts: orderData.products?.length > 0,
            ...
            console.error('🔔 [orderTicketPrinted] Error en respuesta:', response.status);
          }
        } catch (error) {
          console.error('🔔 [orderTicketPrinted] Error al obtener la orden actualizada:', error);
        }
      } else {
    ...
    const registerListener = () => {
      if (webSocketService.isConnected()) {
        console.log('🔔 [orderTicketPrinted] Socket conectado, registrando listeners...');
        ...
        console.log('🔔 [orderTicketPrinted] Listeners registrados correctamente');
      } else {
        console.log('🔔 [orderTicketPrinted] Socket no conectado, esperando conexión...');
        socket.once('connect', () => {
          console.log('🔔 [orderTicketPrinted] Socket conectado ahora, registrando listeners...');
          webSocketService.on('orderTicketPrinted', handleTicketPrinted);
          webSocketService.on('orderUpdatedPending', handleTicketPrinted);
          console.log('🔔 [orderTicketPrinted] Listeners registrados correctamente');
        });
  ...
  }, [selectedTable, selectedOrderByTable, token, handleSetProductsByOrder, orders, tables]);
  ...
  useEffect(() => {
    ...
    console.log('🗑️ [orderDeleted] WebSocket conectado?', webSocketService.isConnected());
    ...
    socket.onAny((eventName, ...args) => {
      ...
      if (eventName === 'orderDeleted') {
        console.log('🔍 [WebSocket Debug] ⚡⚡⚡ EVENTO orderDeleted DETECTADO ⚡⚡⚡');
      }
    });
    ...
    const handleOrderDeleted = async (data: any) => {
      ...
      console.log('🗑️ [orderDeleted] Data completa:', JSON.stringify(data, null, 2));
      ...
      console.log('🗑️ [orderDeleted] Orden seleccionada:', selectedOrderByTable?.id);
      console.log('🗑️ [orderDeleted] Órdenes en mesa seleccionada:', selectedTable?.orders);
      ...
      console.log('🗑️ [orderDeleted] ¿Pertenece a mesa seleccionada?', belongsToSelectedTable);
      ...
      if (belongsToSelectedTable) {
        ...
        if (selectedOrderByTable?.id === orderId) {
          console.log('🗑️ [orderDeleted] Es la orden seleccionada, limpiando...');
          setSelectedOrderByTable(null);
          setConfirmedProducts([]);
          setSelectedProducts([]);
          setSelectedToppingsByProduct({});
        ...
        if (tableId) {
          ...
          if (selectedTable && selectedTable.id === tableId) {
            const updatedTable = {
              ...selectedTable,
              orders: selectedTable.orders?.filter((oId: string) => oId !== orderId) || [],
          ...
          if (tableInStore) {
            ...
            console.log('🗑️ [orderDeleted] Mesa actualizada en store:', finalUpdatedTable.id);
            ...
            console.log('🗑️ [orderDeleted] Mesa no encontrada en store, intentando obtener desde backend...');
            try {
              // Usar el endpoint correcto para obtener la mesa
              const response = await fetch(`${URI_TABLE}/by-room/${selectedTable?.room?.id || ''}`, {
                method: 'GET',
                headers: {
                  Authorization: `Bearer ${token}`,
                },
              ...
              if (response.ok) {
                ...
                console.log('🗑️ [orderDeleted] Mesa obtenida desde backend y actualizada');
              }
            } catch (error) {
              console.error('🗑️ [orderDeleted] Error al obtener mesa desde backend:', error);
            }
          }
        }
      } else {
        console.log('🗑️ [orderDeleted] No se actualiza porque no pertenece a la mesa seleccionada');
      }
    };
    ...
    const registerListener = () => {
      if (webSocketService.isConnected()) {
        console.log('🗑️ [orderDeleted] Socket conectado, registrando listener...');
        webSocketService.on('orderDeleted', handleOrderDeleted);
        console.log('🗑️ [orderDeleted] Listener registrado correctamente');
      } else {
        console.log('🗑️ [orderDeleted] Socket no conectado, esperando conexión...');
        socket.once('connect', () => {
          console.log('🗑️ [orderDeleted] Socket conectado ahora, registrando listener...');
          webSocketService.on('orderDeleted', handleOrderDeleted);
          console.log('🗑️ [orderDeleted] Listener registrado correctamente');
        });
      }
        ...
        if (cancelledOrder) {
          ...
          if (selectedTable) {
            ...
            try {
              ...
              console.error('Error al actualizar la mesa en el backend:', error);
            }
          }
