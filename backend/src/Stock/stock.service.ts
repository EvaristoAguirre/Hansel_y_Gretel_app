...
export class StockService {
  ...
  private async deductPromotionStockWithSelections(
    ...
    // Agrupar selecciones por slotId
    ...
    for (const [slotId, group] of assignmentsBySlot.entries()) {
      ...
      if (totalProductsSelected !== group.totalQuantity) {
        ...
        throw new BadRequestException(
          `Slot "${slot.name}" requires ${group.totalQuantity} product(s), but ${totalProductsSelected} were provided`,
        );
      }

      // Procesar cada selección individual
      for (const selection of slotSelections) {
        // Cada selección puede tener 1 o más productos
