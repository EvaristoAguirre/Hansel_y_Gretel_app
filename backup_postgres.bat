...
set BACKUP_DIR="C:\Users\hanse\Backups Hansel y Gretel sql"
...
"C:\Program Files\PostgreSQL\17\bin\psql.exe" -U %PGUSER% -h %PGHOST% -p %PGPORT% -d %PGDATABASE% -c "SELECT * FROM %TABLE_NAME% WHERE date >= (CURRENT_DATE - INTERVAL '14 days') AND date < (CURRENT_DATE - INTERVAL '7 days');" -o "%BACKUP_FILE%"
...
"C:\Program Files\PostgreSQL\17\bin\psql.exe" -U %PGUSER% -h %PGHOST% -p %PGPORT% -d %PGDATABASE% -c "\copy (SELECT * FROM %TABLE_NAME% WHERE date >= (CURRENT_DATE - INTERVAL '14 days') AND date < (CURRENT_DATE - INTERVAL '7 days')) TO '%CSV_FILE%' WITH CSV HEADER;" >> %LOG_FILE% 2>&1
...
move "%CSV_FILE%" "G:\Mi unidad\Backups Hansel y Gretel"
